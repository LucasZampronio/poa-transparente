...
export async function fetchTceObras(year) {
...
export async function fetchTceCoordinates(idObra) {
...
function mapSector(familia) {
    const f = familia.toUpperCase();
    if (f.includes('SANEAMENTO') || f.includes('AGUA') || f.includes('ESGOTO'))
        return 'SANEAMENTO';
    if (f.includes('PAVIMENTACAO') || f.includes('URBANIZACAO') || f.includes('PRACAS'))
        return 'URBANISMO';
    if (f.includes('EDIFICACOES'))
        return 'ADMINISTRACAO';
    if (f.includes('ILUMINACAO') || f.includes('ENERGIA'))
        return 'URBANISMO';
    if (f.includes('EDUCACAO') || f.includes('ESCOLA'))
        return 'EDUCACAO';
    if (f.includes('SAUDE') || f.includes('HOSPITAL'))
        return 'SAUDE';
    return 'URBANISMO';
}
export async function syncTceObras(year) {
