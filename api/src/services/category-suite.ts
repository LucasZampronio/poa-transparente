...
type AccessLevel = 'live' | 'open-source' | 'api-key' | 'conecta' | 'municipal-system';
...
const SOURCES: Record<string, SourceRef> = {
  localDataset: {
    ...
    access: 'live',
    summary:
      'Base local do projeto com contratos, valores, orgaos, empresas, bairros e datas usadas no painel operacional.',
  },
  ibgeCities: {
    id: 'ibge-cities',
    ...
    access: 'live',
    summary:
      'Base consolidada com populacao, escolarizacao, IDHM, mortalidade infantil, receitas, despesas e PIB per capita.',
  ...
  datasus: {
    ...
    access: 'open-source',
    summary:
      'Conjunto oficial de informacoes de saude com estatisticas vitais, morbidade, internacoes, imunizacao e indicadores municipais.',
  ...
  inep: {
    ...
    access: 'open-source',
    summary:
      'Base oficial para censo escolar, indicadores educacionais, fluxo, infraestrutura e desempenho por municipio.',
  ...
  siconfi: {
    ...
    access: 'open-source',
    summary:
      'API aberta em JSON com receitas, despesas, resultados fiscais e matriz contabil para estados e municipios.',
  ...
  transparencyApi: {
    ...
    access: 'api-key',
    summary:
      'API oficial com beneficios ao cidadao, despesas, convenios, contratos, emendas, licitacoes e outras consultas federais.',
  ...
  snis: {
    ...
    access: 'open-source',
    summary:
      'Base nacional de saneamento com indicadores de agua, esgoto, residuos e drenagem para analise municipal.',
  ...
  cadUnico: {
    ...
    access: 'conecta',
    summary:
      'API do CadUnico com faixa de renda, local de domicilio, idade e Bolsa Familia, com restricao de acesso para municipios.',
...
const CATEGORY_META: Record<string, CategoryMeta> = {
  saude: {
    ...
    rationale:
      'A leitura do setor combina panorama demografico do municipio, pressao assistencial e estrutura da rede para orientar financiamento e acesso.',
  },
  educacao: {
    key: 'educacao',
    label: 'Educacao',
    ...
    rationale:
      'A leitura do setor cruza tamanho da rede, permanencia, desempenho e capacidade de oferta para sustentar metas do sistema municipal.',
  },
  mobilidade: {
    key: 'mobilidade',
    label: 'Mobilidade',
    ...
    rationale:
      'O setor exige ver o territorio, os tempos de deslocamento e o risco viario junto com a execucao fisica dos contratos.',
  },
  infraestrutura: {
    key: 'infraestrutura',
    ...
    rationale:
      'Infraestrutura precisa ser lida como combinacao de investimento, expansao de cobertura e manutencao da cidade.',
  },
  'assistencia-social': {
    ...
    rationale:
      'A leitura do setor prioriza renda, cobertura de programas e capilaridade territorial da rede de protecao.',
  },
};
...
function resolveCategoryMeta(category: string): CategoryMeta {
  ...
  if (normalized === 'saude') return CATEGORY_META.saude;
  if (normalized === 'educacao') return CATEGORY_META.educacao;
  if (normalized === 'mobilidade') return CATEGORY_META.mobilidade;
  if (normalized === 'infraestrutura') return CATEGORY_META.infraestrutura;
  if (normalized === 'assistencia-social') return CATEGORY_META['assistencia-social'];
...
async function getTerritorialBreakdown(category: string) {
...
async function getMonthlySeries(category: string) {
...
function buildLiveSectorIndicators(category: string, overview: CategoryOverview): Indicator[] {
  ...
  return [
    ...
    },
    {
      ...
      description:
        'Proxy do valor monitorado no setor em relacao a populacao estimada do municipio.',
      unit: 'BRL',
      availability: 'live',
...
function buildCommonMunicipalIndicators(): Indicator[] {
  return [
    {
...
function buildHealthGroups(category: string, overview: CategoryOverview): IndicatorGroup[] {
  return [
    {
      id: 'health-context',
      title: 'Contexto demografico e social',
      description:
        'Leitura de tamanho populacional, densidade urbana e condicoes socioeconomicas que pressionam a rede municipal.',
      indicators: [
        ...buildCommonMunicipalIndicators().filter((indicator) =>
          ['population-census', 'population-estimate', 'density', 'idhm', 'gdp-per-capita'].includes(
            indicator.id
          )
        ),
      ],
    },
    {
      ...
      description:
        'Indicadores clinicos e de acesso essenciais para acompanhar o estado de saude e a capacidade de resposta do sistema.',
      indicators: [
        {
          id: 'infant-mortality',
        ...
        },
        {
          ...
          dimension: 'Morbidade',
          description:
            'Internacoes potencialmente evitaveis pela atencao primaria, usadas para qualificar a rede basica.',
          unit: 'rate',
          availability: 'open-source',
          sourceIds: ['datasus'],
          value: null,
    ...
    },
    {
      ...
      description:
        'Capacidade instalada e esforco de financiamento combinados com a carteira atual de contratos do projeto.',
      indicators: [
...
function buildEducationGroups(category: string, overview: CategoryOverview): IndicatorGroup[] {
  return [
    {
      id: 'education-access',
      title: 'Acesso e contexto educacional',
      description:
        'Indicadores basicos para acompanhar o tamanho do municipio, escolarizacao e contexto economico da rede.',
      indicators: [
        {
          id: 'schooling-6-14',
        ...
        ...buildCommonMunicipalIndicators().filter((indicator) =>
          ['population-census', 'population-estimate', 'idhm', 'gdp-per-capita'].includes(
            indicator.id
          )
        ),
      ],
    },
    {
      ...
      description:
        'Pacote de indicadores que orienta metas de permanencia, fluxo escolar e aprendizagem da rede municipal.',
      indicators: [
        {
          id: 'ideb',
    ...
    },
    {
      ...
      description:
        'Leitura do esforco financeiro do municipio e da carteira atual de contratos do setor no painel.',
      indicators: [
...
function buildMobilityGroups(category: string, overview: CategoryOverview): IndicatorGroup[] {
  return [
    {
      id: 'mobility-territory',
      title: 'Territorio e deslocamento',
      description:
        'Leitura do espaco urbano, da densidade e das bases para medir deslocamento e pressao sobre a rede.',
      indicators: [
        ...buildCommonMunicipalIndicators().filter((indicator) =>
          ['population-estimate', 'density', 'gdp-per-capita'].includes(indicator.id)
        ),
        {
          id: 'commute-time',
    ...
    },
    {
      ...
      description:
        'Indicadores operacionais para acompanhar acidentes, infracoes e condicoes de circulacao.',
      indicators: [
        {
          id: 'prf-accidents',
    ...
    },
    {
      ...
      description:
        'Leitura da carteira local do painel com prioridade para intensidade territorial do investimento.',
      indicators: buildLiveSectorIndicators(category, overview),
    },
  ];
}

function buildInfrastructureGroups(category: string, overview: CategoryOverview): IndicatorGroup[] {
  return [
    {
      id: 'infra-territory',
      title: 'Cobertura urbana e escala territorial',
      description:
        'Indicadores de base para leitura de extensao territorial, densidade e capacidade fiscal do municipio.',
      indicators: [
        ...buildCommonMunicipalIndicators().filter((indicator) =>
          ['population-estimate', 'density', 'gross-revenue', 'gross-expenditure'].includes(
            indicator.id
          )
        ),
      ],
    },
    {
      ...
      description:
        'Indicadores de agua, esgoto, residuos e drenagem que sustentam a leitura estrutural do municipio.',
      indicators: [
        {
          id: 'water-coverage',
    ...
    },
    {
      ...
      description:
        'Execucao local do painel e fontes abertas para acompanhamento da carteira de obras municipais.',
      indicators: [
        ...buildLiveSectorIndicators(category, overview),
        {
          id: 'works-execution',
...
function buildSocialAssistanceGroups(category: string, overview: CategoryOverview): IndicatorGroup[] {
  return [
    {
      id: 'social-vulnerability',
      title: 'Contexto social e vulnerabilidade',
      description:
        'Indicadores de base para leitura de renda, desenvolvimento humano e tamanho da demanda potencial.',
      indicators: [
        ...buildCommonMunicipalIndicators().filter((indicator) =>
          ['population-estimate', 'idhm', 'gdp-per-capita', 'gross-expenditure'].includes(
            indicator.id
          )
        ),
      ],
    },
    {
      ...
      description:
        'Fontes federais para monitorar beneficios ao cidadao e cobertura de programas de renda.',
      indicators: [
        {
        ...
        },
        {
          ...
          description:
            'Faixa de renda, local de domicilio, idade e relacao com o Bolsa Familia a partir do CadUnico.',
          unit: 'profile',
          availability: 'conecta',
          sourceIds: ['cadUnico'],
          value: null,
    ...
    },
    {
      ...
      description:
        'Carteira local de contratos do projeto articulada com a leitura de despesa e protecao social.',
      indicators: buildLiveSectorIndicators(category, overview),
    },
  ];
}

function buildIndicatorGroups(category: string, overview: CategoryOverview) {
  const meta = resolveCategoryMeta(category);

  if (meta.key === 'saude') return buildHealthGroups(category, overview);
  if (meta.key === 'educacao') return buildEducationGroups(category, overview);
  if (meta.key === 'mobilidade') return buildMobilityGroups(category, overview);
  if (meta.key === 'infraestrutura') return buildInfrastructureGroups(category, overview);
...
function getAvailabilitySummary(groups: IndicatorGroup[]) {
  const counters: Record<AccessLevel, number> = {
    live: 0,
    'open-source': 0,
    'api-key': 0,
    conecta: 0,
...
function getSourceList(groups: IndicatorGroup[]) {
